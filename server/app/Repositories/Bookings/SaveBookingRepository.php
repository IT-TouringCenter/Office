...
class SaveBookingRepository{    
	...
	public function SaveTransactionBooking($bookingData,$accountId){
		...
		$transaction = [
			...
			'is_service_charge'=>array_get($bookingInfo,'isServiceCharge'),
			'is_advance'=>0,
			// 'is_commission'=>array_get($commission,'isCommission')==true?1:0,
			'is_commission'=>$accountId==0?0:1,
			'is_active'=>0,
	...
	// Save transaction tour reference
	public function SaveTransactionTourReference($transactionId, $upackId){
		$data = [
			"dt_userpackage_id" => $upackId,
			"transaction_id" => $transactionId
		];
		return $this->TransactionTourReference->insertGetId($data);
	}
